-----------------------------------------------------------------------------------------------
-- Client Lua Script for BetterRunecrafting
...
local BetterRunecrafting = {}

function BetterRunecrafting:new(o)
    o = o or {}
    setmetatable(o, self)
    self.__index = self
    return o
...
function BetterRunecrafting:Init()
    Apollo.RegisterAddon(self)
end

function BetterRunecrafting:OnLoad()
	self.xmlDoc = XmlDoc.CreateFromFile("BetterRunecrafting.xml")
	self.xmlDoc:RegisterCallback("OnDocumentReady", self)
end

function BetterRunecrafting:OnDocumentReady()
...
function BetterRunecrafting:OnInterfaceMenuListHasLoaded()
...
function BetterRunecrafting:OnOpenCraftingResume()
	self:Initialize()
	self:RedrawAll()
	self.tWindowMap["RerollItemTitle"]:SetText(Apollo.GetString("EngravingStation_RuneSlotRerollTitleResume"))
end

function BetterRunecrafting:Initialize()
	if self.wndMain and self.wndMain:IsValid() then
		return
	end

    self.wndMain = Apollo.LoadForm(self.xmlDoc, "BetterRunecraftingForm", nil, self)
...
function BetterRunecrafting:OnRunecrafting_TimerStationCheck() -- Hackish: These are async from the rest of the UI (and definitely can't handle data being set)
	if self.wndMain and self.wndMain:IsValid() then
...
function BetterRunecrafting:OnCloseFromUI(wndHandler, wndControl)
	if wndHandler == wndControl then
		self:OnClose()
	end
end

function BetterRunecrafting:OnClose()
	if self.wndMain and self.wndMain:IsValid() then
...
function BetterRunecrafting:OnRedrawAllFromUI()
	self:RedrawAll()
end

function BetterRunecrafting:OnRuneCreationElementToggle(wndHandler, wndControl)
...
function BetterRunecrafting:RedrawAll(itemToOpenTo)
...
function BetterRunecrafting:RedrawRerollBlocker(tCurrentCraft)
...
function BetterRunecrafting:RedrawEquipment(itemToOpenTo)
...
function BetterRunecrafting:RedrawCreation()
	...
	for idx, tCurrSchematic in pairs(CraftingLib.GetSchematicList(CraftingLib.CodeEnumTradeskill.BetterRunecrafting, eElementFilter, nil, false)) do
...
function BetterRunecrafting:DrawCreateRuneContents(wndParent)
...
function BetterRunecrafting:OnTypeTierItemCheck(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	self:DrawCreateRuneContents(wndParent)
	self:ResizeCreateRunes()
end

function BetterRunecrafting:OnTypeTierItemUncheck(wndHandler, wndControl)
	local wndParent = wndHandler:GetData()
	wndParent:FindChild("TypeTierItemContainer"):DestroyChildren()
	self:ResizeCreateRunes()
end

function BetterRunecrafting:ResizeCreateRunes()
...
function BetterRunecrafting:OnUpdateInventory()
	self:HelperRefreshBottomMaterials()
	self:RedrawAll()
end

function BetterRunecrafting:HelperRefreshBottomMaterials()
...
function BetterRunecrafting:OnRuneCreationChatBtn(wndHandler, wndControl)
...
function BetterRunecrafting:OnNewRuneMoreInfoCheck(wndHandler, wndControl) -- NewRuneMoreInfoBtn, data is tSchematicInfo
	if wndHandler ~= wndControl then
		return
	end
...
function BetterRunecrafting:OnNewRuneMoreInfoUncheck(wndHandler, wndControl)
	if wndHandler ~= wndControl then
		return
	end
...
function BetterRunecrafting:ResetRuneCreationBottom()
	local wndParent = self.wndMain:FindChild("RuneCreationContainer"):FindChild("RuneCreationBottom")
	wndParent:FindChild("RuneCreationMaterialsList"):DestroyChildren()
...
function BetterRunecrafting:OnRuneCreationCraftBtn(wndHandler, wndControl) -- RuneCreationCraftBtn
	local tSchematicInfo = wndHandler:GetData()
...
function BetterRunecrafting:OnCloseSummary()
...
function BetterRunecrafting:NewEquipmentItem(itemSource, wndParent, bEquipped)
...
function BetterRunecrafting:OnAppendConfirmYes(wndHandler, wndControl)
	local itemSource = wndHandler:GetData()
	CraftingLib.AddRuneSlot(itemSource)
	self:OnCurrentConfirmPopupClose()
end

function BetterRunecrafting:OnRerollConfirmYes(wndHandler, wndControl)
...
function BetterRunecrafting:OnRerollElementPickedBtn(wndHandler, wndControl) -- From RerollElementLeftBtn or RerollElementRightBtn
	self:HelperPostRerollTransition(self.tWindowMap["RerollDoneCastBarBG"])
end

function BetterRunecrafting:OnRerollElementAbandonBtn(wndHandler, wndControl)
	self:HelperPostRerollTransition(self.tWindowMap["RerollDoneCastBarBG"])
end

function BetterRunecrafting:HelperPostRerollTransition(wndCastBar)
	self.tWindowMap["RerollBlockerHoloBG"]:Show(false)
	self.tWindowMap["RerollBlocker"]:Show(true, true)
	wndCastBar:Show(true)
end

function BetterRunecrafting:OnUnlockConfirmYes(wndHandler, wndControl)
	local nSlotIndex = wndHandler:GetData()[1]
	local itemSource = wndHandler:GetData()[3] -- Data is passed along, origates from RuneSlotBtn and is { nSlotIndex, tCurrRuneSlot, itemSource }
...
function BetterRunecrafting:OnClearConfirmYes(wndHandler, wndControl)
	local nSlotIndex = wndHandler:GetData()[1]
	local itemSource = wndHandler:GetData()[3] -- Data is passed along, origates from RuneSlotBtn and is { nSlotIndex, tCurrRuneSlot, itemSource }
...
function BetterRunecrafting:OnRuneSlotBtn(wndHandler, wndControl) -- RuneSlotBtn of RuneSlotLockedItem
...
function BetterRunecrafting:OnRuneSlotAppendBtn(wndHandler, wndControl) -- RuneSlotAppendBtn of RuneSlotAppendItem
	if not self.bAllowClicks then
		return
	end
...
function BetterRunecrafting:OnRuneSlotLockedBtn(wndHandler, wndControl) -- RuneSlotBtn of RuneSlotLockedItem
	if not self.bAllowClicks then
		return
	end
...
function BetterRunecrafting:OnRunePickerItemBtn(wndHandler, wndControl)
...
function BetterRunecrafting:DrawRuneRerollConfirm(wndHandler, itemSource, nSlotIndex)
	if not self.bAllowClicks then
		return
	end
...
function BetterRunecrafting:DrawAddRuneConfirm(wndHandler, itemSource, nSlotIndex, itemRune)
	self:OnCurrentConfirmPopupClose()
...
function BetterRunecrafting:DrawRuneReplaceConfirm(wndHandler, itemRune)
...
function BetterRunecrafting:OnCurrentConfirmPopupClose(wndHandler, wndControl)
...
function BetterRunecrafting:OnRunecrafting_ConfirmPopupDelay()
	self.bAllowClicks = true
end

function BetterRunecrafting:HelperRepositionConfirmPopup(wndOrigin)
...
function BetterRunecrafting:OnRuneSlotQueryDragDrop(wndHandler, wndControl, x, y, wndSource, strType, nDragInventoryItemIdx, eResult)
...
function BetterRunecrafting:OnRuneSlotDragDrop(wndHandler, wndControl, x, y, wndSource, strType, nDragInventoryItemIdx, bDragDropHasBeenReset)
...
function BetterRunecrafting:OnAddConfirmYes(wndHandler, wndControl) -- Potentially from drag drop or from picker
	local nSlotIndex = wndHandler:GetData()[1]
...
function BetterRunecrafting:OnReplaceConfirmYes( wndHandler, wndControl, eMouseButton )
	local nSlotIndex = wndHandler:GetData()[1]
	local itemRune = wndHandler:GetData()[2] -- GOTCHA: This is an item object, not the normal table of data
	local itemSource = wndHandler:GetData()[3]
...
function BetterRunecrafting:HelperCheckElementMatch(tItemRuneInfo, tRuneSlot)
...
function BetterRunecrafting:HelperPrereqFailed(tCurrItem)
	return tCurrItem and tCurrItem:IsEquippable() and not tCurrItem:CanEquip()
end

function BetterRunecrafting:HelperBuildItemTooltip(wndArg, itemCurr)
...
function BetterRunecrafting:LoadByName(strForm, wndParent, strCustomName)
...
local BetterRunecraftingInst = BetterRunecrafting:new()
BetterRunecraftingInst:Init()
